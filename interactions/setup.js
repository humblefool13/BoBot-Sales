...
async function getOSdata(slug) {
  ...
  const address = response?.collection?.primary_asset_contracts.length
...
module.exports = {
  ...
  async interact(client, interaction) {
    try {
      if (interaction.inGuild()) {
        ...
        if (!permissions.has(PermissionsBitField.Flags.ManageRoles))
          return interaction.reply({
            content: `I do not have the \`MANAGE_ROLES\` permission . Please grant me the permission before using this command.`,
            ephemeral: true,
          });
        if (!permissions.has(PermissionsBitField.Flags.ManageWebhooks))
          return interaction.reply({
            content: `I do not have the \`MANAGE_WEBHOOKS\` permission . Please grant me the permission before using this command.`,
            ephemeral: true,
          });
        if (!permissions.has(PermissionsBitField.Flags.ManageChannels))
          return interaction.reply({
            content: `I do not have the \`MANAGE_CHANNELS\` permission . Please grant me the permission before using this command.`,
            ephemeral: true,
          });
        if (!permissions.has(PermissionsBitField.Flags.UseExternalEmojis))
          return interaction.reply({
            content: `I do not have the \`USE_EXTERNAL_EMOJIS\` permission . Please grant me the permission before using this command.`,
            ephemeral: true,
          });
        if (!permissions.has(PermissionsBitField.Flags.SendMessages))
          return interaction.reply({
            content: `I do not have the \`SEND_MESSAGES\` permission . Please grant me the permission before using this command.`,
            ephemeral: true,
          });
      }
      ...
      const opensea_slug = opensea_link
        .trim()
        .slice(opensea_link.lastIndexOf("/") + 1);
      let OS_data;
      do {
      ...
      if (!findsubs.length)
        return interaction.editReply({
          embeds: [
            MakeEmbed(
              "You do not have a subscription. Please contact us at our [support server](https://discord.gg/HweZtrzAnX) to get a subscription."
            ),
          ],
          ephemeral: true,
        });
      if (findsubs.length === findconfigs.length && !findcollection)
        return interaction.editReply({
          embeds: [
            MakeEmbed(
              "You have a collection setup for every subscription you have. You can either\n1) Get a new subscription for a new collection,\n2) Replace an exising collection by a new one using `/replace` command. This will stop posting for old collection and old channels will stop working."
            ),
          ],
          ephemeral: true,
        });
      ...
      if (chain === "ETH") {
        ...
        if (!ME_link)
          return interaction.editReply({
            embeds: [
              MakeEmbed(
                "Providing a Magic Eden link is necessary for Solana collections."
              ),
            ],
            ephemeral: true,
          });
      ...
      if (!findcollection) {
        ...
        const stats_channel = await interaction.guild.channels.create({
          ...
          parent: category,
          topic:
            "Stats channel Managed by BoBot Labs Sales Bot : https://discord.gg/HweZtrzAnX",
        ...
        const sales_channel = await interaction.guild.channels.create({
          ...
          parent: category,
          topic:
            "Sales channel Managed by BoBot Labs Sales Bot : https://discord.gg/HweZtrzAnX",
          permissionOverwrites: [
            {
              id: client.user.id,
              allow: [
        ...
        const listings_channel = await interaction.guild.channels.create({
          ...
          parent: category,
          topic:
            "Listings Channel Managed by BoBot Labs Sales Bot : https://discord.gg/HweZtrzAnX",
          permissionOverwrites: [
            {
              id: client.user.id,
              allow: [
        ...
        if (role.id === interaction.guild.id) {
          await sales_channel.permissionOverwrites.edit(interaction.guild.id, {
            ViewChannel: true,
            SendMessages: false,
          ...
          await listings_channel.permissionOverwrites.edit(
          ...
          await stats_channel.permissionOverwrites.edit(interaction.guild.id, {
            ViewChannel: true,
            SendMessages: false,
            ReadMessageHistory: true,
            AddReactions: true,
          ...
          await sales_channel.permissionOverwrites.create(role.id, {
            ViewChannel: true,
            SendMessages: false,
            ReadMessageHistory: true,
            AddReactions: true,
          ...
          await listings_channel.permissionOverwrites.create(role.id, {
            ViewChannel: true,
            SendMessages: false,
            ReadMessageHistory: true,
            AddReactions: true,
          ...
          await stats_channel.permissionOverwrites.create(role.id, {
            ViewChannel: true,
            SendMessages: false,
            ReadMessageHistory: true,
            AddReactions: true,
        ...
        const sales_webhook = await sales_channel.createWebhook({
          name: "BoSales S",
          avatar:
            "https://cdn.discordapp.com/attachments/1003737982106353744/1003738097084801114/Discord-1-modified_1.png",
        ...
        const listings_webhook = await listings_channel.createWebhook({
          ...
          avatar:
            "https://cdn.discordapp.com/attachments/1003737982106353744/1003738097084801114/Discord-1-modified_1.png",
          reason:
            "This webhook was created by BoBot Sales Bot to post listings.",
        });
        ...
        const everyonePermissions =
          interaction.guild.roles.everyone.permissions;
        if (
          !everyonePermissions.has(PermissionsBitField.Flags.UseExternalEmojis)
        ) {
          await interaction.guild.roles.everyone.permissions.add(
        ...
        findcollection.save().then(() => {
          ...
          if (
            ...
            interaction.guild.roles.everyone.permissions.add(
              PermissionsBitField.Flags.UseExternalEmojis
            );
          }
          return interaction.editReply({
            embeds: [
              MakeEmbed(
                `You have re-setup your configuration for ${findcollection.opensea_slug}. The old channels will stop working and the bot will start with the freshly made channels - <#${stats_channel.id}>, <#${sales_channel.id}> & <#${listings_channel.id}>. The bot will start posting sales and listings soon.\n\nYou can rename the channel or move them to other categories but please do not make any changes in channels' permissions else it might affect functionality of bot.`
              ),
            ],
            ephemeral: true,
          });
      ...
      if (interaction.deferred || interaction.replied) {
        await interaction.followUp({
          content:
            "I am facing some issues, the dev has been informed. Please try again in some hours.",
          embeds: [],
          components: [],
          ephemeral: true,
        });
